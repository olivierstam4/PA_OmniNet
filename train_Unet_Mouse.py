...
HDF5_OUTPUT_PATH = '../Datasets/optoacousticsparse/mice_full_recon_256.mat'
GAMMA = 0.98
MOMENTUM = 0.9
STEP_SIZE = 1
LEARNING_RATE = 1e-4
...
BATCH_SIZE = 1
START_FILTERS = 32
MODEL_SAVE_PATH = "unet_model_mouse16_ssim.pth"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    for epoch in range(EPOCHS):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{EPOCHS} [Training]") as pbar:
            for x, y in train_loader:
                ...
                # clip_value = .03
                y_pred = model(x)
                ssim_value = ssim(y_pred, y)
                loss = 1 - ssim_value
                # alpha = 0.84
                # mse_value =  F.mse_loss(y_pred, y)
                # ssim_value = ssim(y_pred, y.to(DEVICE))
        ...
        with tqdm(total=len(val_loader), desc=f"Epoch {epoch + 1}/{EPOCHS} [Validation]") as pbar:
            with torch.no_grad():
                for x, y in val_loader:
                    ...
                    # ssim_value = ssim(y_pred, y.to(DEVICE))
                    # loss = (1 - alpha) * mse_value + alpha * (1 - ssim_value)
                    ssim_val =ssim(y_pred, y.to(DEVICE))
                    loss = 1 - ssim_val
                    # loss = F.mse_loss(y_pred, y) + (1-ssim_val) * alpha
                    val_loss += loss.item()
                    logger.info(f'Validation: {val_loss:.4f}')
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       "Unet_ssim_mouse16.pth")
            logger.info(f"Validation loss improved. Model saved as Unet_mouse16_ssim_bestmodel")
        else:
        ...
        if (epoch + 1) % 50 == 0:
            checkpoint_path = os.path.join("checkpoints", f"Unet_mouse16_full{epoch + 1}.ckpt")
