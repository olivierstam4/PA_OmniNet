...
class LightningModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #loss = 1 - ssim_value
        #loss = F.mse_loss(y_pred, y) + (1-ssim_val) * alpha
        self.log('train_loss', loss, on_epoch=True, prog_bar=True)
        #logging.info(f'loss = {(1-alpha) * mse_value} + {alpha * (1- ssim_value)} with an ssim of {ssim_value} and a mse of {mse_value}')
        return loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        test_loss = self.ssim(y_pred, y.to(DEVICE))
    ...
    def on_train_end(self):
        ...
        model_path = os.path.join(logger_dir, "firstssimmodeltogether_msevalidation.pth")
...
if __name__ == "__main__":
    ...
    logging.info(f"CUDA Available: {cuda_available}\nCurrent CUDA Device: {current_device if cuda_available else 'N/A'} \nGPU Name: {gpu_name}")
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='PAOmni_MSFD_alpha_{epoch:02d}-{val_loss:.4f}_500',
        save_top_k=1,
        mode='min'
    )
