...
BATCH_SIZE = 128
START_FILTERS = 32
MODEL_SAVE_PATH = "unet_model_MFSD_patient.pth"
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            early_stop_counter = 0
            torch.save(model.state_dict(), "best_model_MFSD.pth")
            logger.info(f"Validation loss improved. Model saved as best_model_MFSD.pth.")
        else:
        ...
        if (epoch + 1) % 50 == 0:
            checkpoint_path = os.path.join("checkpoints", f"Unet_MFSD_patient_{epoch + 1}.ckpt")
